...
import { useUpdateQuiz } from "../api/use-update-quiz";
...
export function QuizUpdateForm({ data }: QuizUpdateFormProps) {
  ...
  const handleSubmit = (values: AddQuizInput) => {
    mutate(
      ...
      {
        onSuccess: () => {
          router.push(`/dashboard/quizzes?active_lesson=${lessonId}`);
        },
      }
    );
  };
