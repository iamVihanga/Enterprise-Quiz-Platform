...
import {
  inviteAdminSchema,
  type InviteAdminSchema,
} from "../schemas/invite-admin";
import {
  Form,
  FormControl,
  FormField,
...
export function InviteAdmin() {
  ...
  const onSubmit = (values: InviteAdminSchema) => {
    mutate(values, {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button icon={<PlusCircle className="size-4" />}>
          Invite Class Admin
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Invite to new class Admin</DialogTitle>
          <DialogDescription>
            Join a new admin to selected class
          </DialogDescription>
        </DialogHeader>
